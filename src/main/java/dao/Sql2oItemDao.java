...
public class Sql2oItemDao implements ItemDao {
  ...
  public void add(Item item) {
    ...
    String sql = "INSERT INTO items (itemName, cost, tip, userId, receiptId) VALUES (:itemName, :cost, :tip, :userId, :receiptId)";

    try (Connection con = sql2o.open()) {
      int id = (int) con.createQuery(sql, true)
              .addParameter("itemName", item.getItemName())
              .addParameter("cost", item.getCost())
              .addParameter("tip", item.getTip(item.getPercent()))
              .addParameter("userId", item.getUserId())
  ...
  public void update(int id, String itemName, int userId) {
    String sql = "UPDATE items SET (itemName, userId) = (:itemName, :userId) WHERE id=:id";
